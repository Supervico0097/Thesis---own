...
class SyntheticStreamingDataGenerator:
    ...
    def create_users(self, week_no):
        ...
        for _ in range(num_users_to_add):
            user = {
                ...
                "age": int(
                    ((np.random.lognormal(mean=1, sigma=0.1, size=1)) * 20 - 25)[0]
                ),
            ...
            for _ in range(int(np.random.choice([0, 1, 2, 3], p=[0.2, 0.4, 0.3, 0.1]))):
                user["favorite genres"].append(
                    np.random.choice(
                        a=self.probabilities_genre_df.index.to_list(),
                        p=self.probabilities_genre_df.user.to_list(),
                    )
                )
            user["favorite genres"] = list(
                dict.fromkeys(user["favorite genres"])
            )  # remove duplicates
            ...
            for _ in range(random.randrange(5)):
                for _ in range(20):
                    ...
                    if (
                        choice["continent"] == user["continent"]
                        and choice["genre"] in user["favorite genres"]
                        and choice["is_famous"]
                        ...
                        choice["genre"] in user["favorite genres"]
                        and choice["is_famous"]
                        and random.uniform(0, 1) < 0.4
                    ):
                        to_add = True
            ...
            user["favorite artists"] = list(
                dict.fromkeys(user["favorite artists"])
            )  # remove duplicates

            # add favorite songs - first go through favorite artists
            for i in user["favorite artists"]:
                favorite_artist_list = [
                    song for song in self.song_list if song["artist_id"] == i
                ]
            ...
            user["favorite songs"] = list(
                dict.fromkeys(user["favorite songs"])
            )  # remove duplicates
    ...
    def create_artists(self, week_no):
        num_artists_to_add = round(
            (11.17315 + 0.3660266 * week_no + 0.009279995 * week_no**2)
            * (self.artists_number / 5976.23302825)
        )
    ...
    def generate_streams(self, week_no):
        ...
        for user_i in range(len(self.user_list)):
            ...
            if (
                ...
                and len(self.song_list) > self.avg_songs_sub
            ):
                ...
                and len(premium_songs) < self.avg_songs_unsub
            ):
            ...
            if (
                random.uniform(0, 1) < self.p_random_songs_stream and list_songs
                ...
                for _ in range(int(n_songs)):  # go through songs
                    ...
                    # if singer is famous, song is famous, same genre
                        ...
                        and random.uniform(0, 1) < self.p_fav_art_sng_gnr
                    ):
                        user = self.add_stream(
                            self.user_list[user_i], random_song["song_id"], week_no
                        )
                    ...
                    # if  same genre
                        ...
                        random_song["genre"]
                        in self.user_list[user_i]["favorite genres"]
                    ) and random.uniform(0, 1) < self.p_fav_gnr:
                        user = self.add_stream(
                            self.user_list[user_i], random_song["song_id"], week_no
                        )
                        self.user_list[user_i] = user
            ...
            if (
                random.uniform(0, 1) < self.p_favorite_playlist
                and self.user_list[user_i]["favorite songs"]
            ):  # check if user will access favorite songs
    ...
    def add_stream(self, user, i, week_no):
        ...
        if (
            ...
            > self.min_streams_to_famous_artist
        ):
